    ...
    def step(self, action_dict):
        ...
        for i, agent in enumerate(self.team):
            for j in range(len(agent_pos_maps)):
                if j == i:
                    agent_pos_maps[j][agent.position[Y], agent.position[X]] = 2/3
                else:
                    agent_pos_maps[j][agent.position[Y], agent.position[X]] = 1/3
            agents_pos_map_g[agent.position[Y], agent.position[X]] = agent.agent_id / 3.0
        # coverable area
        map_cc = copy.deepcopy(self.map.map)
        cov_cc = copy.deepcopy(self.map.coverage)
        coverable_area = (map_cc + cov_cc) == 0
